...
async function fetchUiVersions() {
  const res = await fetch('https://data.jsdelivr.com/v1/package/npm/@auraui/ui');
  const { versions } = (await res.json()) as { versions: string[] };
...
function setUiVersion(version: string) {
  ...
  store.setImportMap({
    imports: {
      '@auraui/ui': `https://fastly.jsdelivr.net/npm/@auraui/ui@${version}/dist/auraui-ui.full.min.mjs`,
    },
  });
  // 加载组件库的全量样式
  previewOptions.headHTML = `<link rel="stylesheet" href="https://fastly.jsdelivr.net/npm/@auraui/ui@${version}/dist/style/index.css">`;
}
  ...
  <div v-if="isMounted">
    ...
    <Teleport to=".VPNavBarSearch">
      <div class="flex items-center text-14px">
        <label class="playground-label">AuraUI: </label>
...
.playground-select {
  ...
  margin-left: 8px;
  appearance: auto;
  border: 1px solid rgb(var(--au-color-bd_base));
}
</style>
